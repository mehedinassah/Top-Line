    ...
    <section className="border-b border-neutral-200 bg-white w-full">
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-10 py-10 sm:py-14 md:py-16 lg:py-20">
        ...
        <div className="relative">
          {/* Gradient overlay for mobile to create seamless effect */}
          <div className="absolute inset-0 pointer-events-none sm:hidden">
            <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-transparent via-transparent to-black/5 z-10"></div>
            <div className="absolute top-1/3 left-0 right-0 h-32 bg-gradient-to-b from-black/5 via-transparent to-black/5 z-10"></div>
            <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-transparent via-transparent to-black/5 z-10"></div>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 sm:gap-2 md:gap-3">
            {categories.map((category) => (
              <Link key={category.slug} href={`/categories/${category.slug}`}>
                <div className="group relative aspect-[5/3] sm:aspect-square overflow-hidden bg-neutral-100 cursor-pointer sm:gap-2">
                  {/* Image */}
                  <Image
                    src={category.image}
                    alt={category.title}
